...
import ex04controlstatement.gogodan;
...
class MyClass {
	...
	@Override
	/*
	 객체1, 2 의 x,y값을 동시에 비교해서 둘다 동일할떄 같은 객체로 
	 판단하도록 오버라이딩 처리 
	 */
	public boolean equals(Object obj) {
		...
		if( obj instanceof MyClass) {
			//MyClass 형으로 다운캐스팅 진행
			MyClass mc = (MyClass)obj;
			//두 객체의 멤버변수의 값을 실제로 비교 
			if(mc.data == this.data) {
				return true;//값이 동일할 떄 true
			}
			else {
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		//toString()메소드의 오버라이딩 한 내용을 출력
		System.out.println("[두 객체의 toString()=>"+ mc1.toString());
		System.out.println("[두 객체의 toString()=>"+ mc2);
