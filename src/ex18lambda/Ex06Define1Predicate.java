...
/*
 ...
 Predicate : 매개변수 있고, 리턴은 boolean값으로 변환 
 */
public class Ex06Define1Predicate {
	public static void main(String[] args) {
		/*
 		Interface Predicate<E>{
 		boolean test(T t); //추상메소드 
 		}
 		:전달된 인자를 대상으로 true, false 를 판단할때
 		사용하는 추상메소드를 정의하고 있다.
		 */
...
//		System.out.println("=====================");
	...
	public static double avg (Predicate<Human> ph, List<Human> li) {
		...
		for(Human h : li) {//li 컬렉션의 크기만큼 반복
			/*
			 첫번쨰 호출에서는 남자일 떄 true를 반환한다. 즉 남자의 점수가  카운트됨. 
			 두번째 호출은 여자. 
			 */
			if(ph.test(h)==true) {
				pCount++;//인원수 증가
