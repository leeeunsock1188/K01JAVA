...
/*
 함수형 인터페이스 
 	...
 	람다식으로 구현할 추상메소드를 선언한다. 이를 람다 식을 
 	만들기위한 함수형인터페이스 한다.
 	※ 함수형 인테퍼이스에는 오직 하나의 추상메소드만 선언할 수 있다. 
@FunctionalInterface 
	:함수형 인터페이스의 조건을 갖추었는지에 대한 검사를
	컴파일러에게 요청하는 역할을 하는 어노테이션이다. 
 */
@FunctionalInterface // 함수형 인터페이스라고 말해주는것 임 
interface IKosmo4 { //하나만 올수있음. 
	void studyLambda(String str);// [추상메소드]
//	void studyLambda1(String str); 두개이상쓰면 [에러발생함]
}
