...
public class E05TwoDimArray02 {
	...
	public static void main(String[] args) {
		...
		for(int i =0; i<3 ; i++) {
			for(int  j=0; j<4; j++) {
				arr2Dim[i][j] = i+j; // {0.1.2.3} , {1,2,3,4}, {2,3,4,5}
			}
		}
		/*
		 초기화 된 데이터를 출력. 
		 */
		for(int n=0; n<arr2Dim.length; n++) {// 배열의ㅣ 세로길이
...
3. 배열전체요소를 매개변수로 전달된 정수만큼 0~1사이 실수 math.random x 100더하면 
0.xxxxx 오른쪽으로 두개감 
	증가시킨다.
4. 증가되기 전, 후 배열을 출력한다.
 ...
 */
	...
	/*
	 배열을 출력하는 부분이 중복되기 떄문에 이런경우 메소드를
	 생성하여 호출해 주는것이 좋다.
	 */
 static void showArray(int[][] arr2) {
	 ...
	 static void twoDimPlus(int[][]arr2, int plusNum) {
		 for(int i=0; i<arr2.length; i++) {
			 for(int j=0; j<arr2[i].length; j++) {
				 /*
				  매개변수로 전달된 plusNum 만큼 각 요솔를 증가시킴.
				  */
