...
/*
 2] 런타임 에러 
 -컴파일시에는 체크가 안되고 실행시에만 밠생되는 에러
 -RuntimeException 클래스 계열로 NullPointerException이
 대표적인 하위 클래스이다
 - main 메소드에서 무시하기 위해 throws하더라도 오류가 발생되므로
 반드시 예외처리를 해줘야 한다.
 -예외발생시 JVM 은 해당 예외클래스를 인스턴스화해서 프로그램 쪽으로
 전달하게 되고 try ~ catch에ㅐ서 받은후 처리한다.
 */
		...
		try {
			...
			*/
			...
			/*
			 ...
			 printStackTrace() : 예외가 발생할떄의 에러메세지를
			 	그대로 콘솔에 출력해줌. 개발시 훨씬 더 사용빈도 놓음.  
			 */
//			System.out.println("예외메세지:"+ e.getMessage());
			e.printStackTrace(); //활용빈도가 더좋음 
		}
		...
		/*
		 ...
		 :숫자형식의 문자열을 int 로 변환하는 경우 해당 문자열이
		 숫자형식이 아니면 발생되는 에외
		 */
		...
		try {
			...
			String strAge = scanner.nextLine();// <='10살' 형태로 입력 
		...
		try {
			System.out.println("나이를 입력하세요,");
			int intAge = scanner.nextInt();//<= 문자를 입력하면 예외발생 
	...
	떄문에, 여기서는 상속관계가 확인되어 true를 반환한다.
	 */
