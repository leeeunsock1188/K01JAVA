...
public class Ex05HashMapMain {
	...
	public static void main(String[] args) {
		...
		System.out.println("name이라는 키값으로 저장된 이전의 값:"
				+ map.put("name", "홍길동")); //-> 홍길동 반환됨
		/*
		 객체저장
		 	:객체 저장시 기존에 저장된 동일한 key값이 존재하면 
		 	이전 객체가 반환된다. 만약 처음이라면 null 값이 
		 	반환된다.
		 */
...
//			map.put("age",number) ; //[에러발생] value타입이 맞지않음.
			map.put("age",String.valueOf(number)) ; // 10 == "10" 변환 
			map.put("gender","남자") ; 
			map.put("address","가산다지털단지") ;
			...
			/*
			 5-2]키값을 모를때 출력하기
			 	...
			 	Set계열의 컬렉ㄱ션 형태로 가져온다
			 	2.Map 계열의 컬렉에는 처음부터 확장 for 문을 사용하는
			 	것이 불가능하다,
			 	3. 키값을 얻어온후 확장for문을 통해 key값을 하나씩
			 	출력하낟.
			 */
			
			Set<String> keys = map.keySet(); // keySet이라는 메소드에서 얻어와서 
			...
			/*
			 5-3]반복자를 통한 출력
			 : iterator를 통해 반복할떄도 keySet() 으로 키값을 먼저
			 얻어온후 사용할 수 있다. 
			 */
			...
			System.out.println(map.containsKey("남자")? "남자 키값있다":"남자 키값없다");
			System.out.println(map.containsKey("여자")? "여자 키값있다":"여자 키값없다");
			/*
			 7]삭제
			...
			System.out.println("삭제된객ㅊ[:"+ map.remove("age"));
			System.out.println("[age키값을 삭제후 출력]");
			for(String key : keys) {
				System.out.println(String.format("%s:%s" ,key, map.get(key)));
			}
			//8] 전체삭지 : removeALL() 은 없음
			map.clear();
