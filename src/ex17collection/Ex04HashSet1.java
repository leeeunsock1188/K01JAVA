...
/*
 HashSet 켈렉션: Set계열의 인터페이스를 구현한 켈렉션으로 
 -객체가 순서없이 저장된다.
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		set.add(dateObject);
		set.add(number); // <- new Integer(number) 1.4이전 버전에는 빡싱처리를 해야줘함
		set.add(teacher);
		...
		/*
		 5-2] 개발자가 정의한 객체 중복저장
		 	...
		 	오버라이딩 하지 않으면 중복 저장이 허용된다. 아래의 경우 저장에 
		 	성공하여 true가 반환된다.
		 */
		...
		//1-8 ] 삭제 
		System.out.println(set.remove(strObject2)? "strObject2삭제성공":
			"strObject삭제실패");
		...
		//1-9]전체삭제 
		//set.clear(); 
		System.out.println("전체삭지:"+set.removeAll(set));
