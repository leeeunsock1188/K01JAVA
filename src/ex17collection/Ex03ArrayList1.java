...
/*
 ArrayList : List계열의 컬렉션으로 
 	-데이터의 중복 저장이 허용된다.
 	-데이터의 저장 순서를 보장한다.
 	-데이터에 접근할떄 iterator() 혹은 get()을 
 		이용한다.
 	-Array라는 이름처럼 배열의 특성을 가지고있어 
...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		ArrayList list = new ArrayList(); //모든 객체를 저장할수 있는 컬렉션 
		ArrayList list1 = new ArrayList(); //모든 객체를 저장할수 있는 컬렉션 
		 ...
		 */
			...
			list.add("트와이스");
			list.add(3,"워너원");
//			list.add(5,"소방챀ㅋㅋ"); //에러발생. 인덱스는 건너 뛸수없다. 
			list.add(list.size(),"오마이걸");// <- 인덱스에 입력됨. 
		...
		*/
		...
		/*
		 1-6] 포함여부확인
		 	confains(객체명) :찾고자하는 객체가 있는경우 
		 		true를 반환한다.
		 */
		System.out.println(list.contains("백뱅")?"빅뱅 있음": "빅뱅 없음");
		...
		/*
		 ...
		 방법1 : 인덱스로 삭제하기 
		 	remove(인덱스) : 인덱스를 통해서 삭제를 진행한후 
		 		삭제가 완료되면 해당 객체를 반환하낟. 삭제후에는 
		 		인덱스가 자동으로 재부여 된다. 
		 */
		Object obj = list.remove(2);
		System.out.println("삭제된객체:"+ obj);//삭제된 객체가obj로 반환된다. 
