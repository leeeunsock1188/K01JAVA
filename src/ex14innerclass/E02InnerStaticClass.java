...
/*
 ...
 -이와 같이 내부클래스를 static 으로 선언하면 Nested클래스라고 한다,
 -정적 멤버변수 & 멤버메소드를 가질 수 있다.,
 -외부클래스의 인스턴스형 멤버는 사용할수 없다,.
 -외부 클래스명 $내부클래스명.class 파일이 생성된다.

 ※원래 클래스앞에는 static 을 붙일수 없으나 내부클래스에는 허용된다.
 */
class OuterClass {
	//인스턴스형 멤버변수 
	...
	static void outerStaticMethod() {
		...
		/*
			 외부클래스의 정적 멤버 메소드에서는 내부클래스이 정적멤버만
		...
		InnerStaticClass.innerStiaticVar =103;
...
public class E02InnerStaticClass{
	...
	public static void main(String[] args) {
		...
		/*
			 외부 클랫스가 아닌 다른 클래스에서 접근할떄의 방법은 아래와 같다.
		 */

		/*
			 내부정적 클래스의 정적멤버에 접근할떄는 외부클랫의 객체 생성없이
			 클래스명으로 바로 접근이 가능하다.
		 */
		OuterClass.InnerStaticClass.innerStiaticVar =1;
		OuterClass.InnerStaticClass.innerStaticMethod();

		/*
			 내부정적 클래스의 인스턴스멤버에 접근할떄는 객체를 생성한후 
			 참조변수를 통해서 접근해야 한다.
		 */
		...
		/*
			 ...
			 외부클래스의 멤버 변수 (메소드)는 객체생성후 참조변수를 통해
			 접근한다.
		 */
